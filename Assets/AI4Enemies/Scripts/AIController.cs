...
public class AIController : MonoBehaviour
{
    ...
    void InitAnimations()
    {
        ...
        if (dieAnimation != "")
        {
            animation[dieAnimation].wrapMode = WrapMode.ClampForever;
            animation[dieAnimation].layer = 4;
            animation.Stop(dieAnimation); //2011-05-28-1
        ...
        if (hitAnimation != "")
        {
            ...
            animation[hitAnimation].layer = 3;
            animation.Stop(hitAnimation); //2011-05-28-1
        }

        animation[fightAnimation].wrapMode = WrapMode.Once;
        animation[fightAnimation].layer = 2;
        shootAnimationLength = animation[fightAnimation].length;
        animation.Stop(fightAnimation); //2011-05-28-1
        animation[idleAnimation].layer = 1;
        animation.Stop(idleAnimation); //2011-05-28-1
        animation[walkAnimation].layer = 1;
        animation.Stop(walkAnimation); //2011-05-28-1
    }
            ...
            while (isAlive)
            {
                ...
                if (isAttaking && !isHit == true)
                {
                    ...
                    if (EqualRotations(toTargetRotation, myTargetPoint.rotation, targetingTolerance))
                    {
                        ...
                        {
                            animation.CrossFade(fightAnimation);                                                       
                        }
                        else
                        {
                            //animation.Play(fightAnimation);
                            animation.PlayQueued(fightAnimation, QueueMode.PlayNow);
                        }
                        ...
                        if (projectile != null)
                        {
                            ...
                            {
                                proj.rigidbody.AddRelativeForce(Vector3.forward * projectileForce);
                            }
                        }
                        //Debug.Log("shoot");
                        if (playSound)
    ...
    void ObstacleAvoidance()
    {
        ...
        {               
            rigidbody.velocity = rigidbody.velocity + oaMoveDirection.normalized * (movingSpeed) ;
        }
